# Adobe AI Studio — комплексная документация
...
Python sidecar. Цель — запускать генерации (Nano Banana, GPT Image, Kling,
Seedance, Kling Omni, Kling Motion, Topaz Video Upscale, ElevenLabs TTS)
прямо из Adobe и автоматически класть результат на таймлайн / в композицию.

**Почему нельзя ходить из CEP напрямую в backend:**

- Backend закрыт за SuperTokens (header-mode JWT + cookies + refresh-роутинг
  `rid:anti-csrf`, обновление `st-access-token` по `/auth/session/refresh`).
- Под корпоративным MITM-прокси нужен `truststore.SSLContext` (CA из системного
...
- В сумме портировать на JS внутри CEP — 2–3 недели + новые баги. Sidecar
  переиспользует vendored Python-клиент без переписывания.
...
sidecar/                       FastAPI приложение на 127.0.0.1:8765
  app/
    ...
    phygital_client/           vendored backend-клиент (НЕ редактировать)
    workflows/
    ...
    routers/
      health.py                /health
      auth.py                  /auth/recon
      nodes.py                 /nodes, /nodes/video, /nodes/upscale
      jobs.py                  /jobs, /jobs/{id}, /jobs/{id}/download, /jobs/preview-cost
      assets.py                /assets — sha256-дедуп, disk-usage/disk-cache
      account.py               /account/balance — обёртка над credits_info
      clips.py                 /extract-frame, /clip-video — ffmpeg
      enhance.py               /enhance — prompt enhancer (Gemini Text)
      presets.py               /presets — пресеты форм (user_data/presets.json)
    services/
...
cep-premiere/                  CEP-панель Pr (бандл com.phygital.studio.pr)
  ...
  client/
    ...
    components/                Preact + htm компоненты
      ...
      FamilyTabs.js            вкладки семейств Image / Video / Upscale / Voice
      GenerateTab.js           слоты + промпт + параметры + submit (+ VersionPicker)
      HistoryTab.js            фильтр + список job'ов
      SlotPicker.js, SlotList.js
      ModelPicker.js, ScenarioPicker.js
      EnumDropdown.js          custom dropdown (native select обрезается CEP-iframe'ом)
      ParamsAccordion.js       Advanced settings (скрывает version-параметр и 1-опционные enum'ы)
      PresetBar.js             пресеты формы (/presets)
      PromptInput.js
      SubmitButton.js, CostBar.js
      QueueWidget.js           активные job'ы + per-job cancel
    ...
    lib/
      ...
      disk.js, disk_save.js    запись результатов на диск, sniff magic-байтов
      sidecar_token.js         shared-secret панель → sidecar
      slot_labels.js           friendly-имена слотов (init_img → "Initial image")
      param_labels.js          friendly-имена параметров и enum-значений
      slot_schema.js           схема слотов + getVersionParam (Version-дропдаун)
      state.js, toast.js, validation.js, format.js, html.js
...
docs/                          ← этот документ + ARCHITECTURE/AUTH/HANDOFF/INSTALL_*
```
...
└──────────────────────────────────────────┼──────────────────┘
                  ...
                  │  ├─ /jobs, /jobs/{id}, /assets        │
                  │  ├─ /account/balance, /enhance        │
                  │  ├─ /extract-frame, /clip-video       │
                  │  ├─ /presets                          │
...
Текущий обход (QE-рендер минуем полностью):

```
host.jsx находит топовый video/image-клип под playhead'ом активной sequence
  ├─ статичная картинка на дорожке → путь отдаётся напрямую
  └─ видео → пересчёт playhead'а в source-relative секунду
        │
        ▼
client/api.js: POST /extract-frame {source_path, at_sec}
        │
        ▼ (sidecar routers/clips.py)
ffmpeg -ss at_sec -i source_path -frames:v 1 → PNG
        │
        ▼
AppData/PhygitalStudio/asset_uploads/frame_<uuid>.png → {path} панели
```

Аналогично «From Timeline In/Out» / «Selected clip» для видео-слотов:
`POST /clip-video {source_path, in_sec, out_sec}` → re-encode (libx264+aac)
→ `asset_uploads/clip_<uuid>.mp4`.
...
| `GET` | `/nodes/upscale` | — | upscale-ноды (Topaz) + опции |
| `POST` | `/jobs` | `{node_id, params, init_files?}` (+ опц. header `Idempotency-Key`) | `{job_id}` |
...
| `HEAD` | `/jobs/{id}/download` | `?index=0` | заголовки без тела |
| `GET` | `/jobs/{id}/download` | `?index=0` | bytes |
| `DELETE` | `/jobs/{id}` | — | 204 |
| `POST` | `/assets` | multipart `file=@...` | `{sha256, file_obj_id, mime, size}` |
| `GET` | `/assets` | — | `{assets: [...]}` |
| `GET` | `/assets/disk-usage` | — | размер локального кэша |
| `DELETE` | `/assets/disk-cache` | — | очистка локального кэша |
...
| `POST` | `/extract-frame` | `{source_path, at_sec}` | `{path}` — PNG в `asset_uploads/` |
| `POST` | `/clip-video` | `{source_path, in_sec, out_sec}` | `{path, duration_sec, size_bytes}` — клип в `asset_uploads/` |
| `POST` | `/enhance` | `{prompt, ...}` | улучшенный промпт (Gemini Text, node 72) |
| `GET/POST` | `/presets` + `DELETE /presets/{id}` | пресеты формы | `user_data/presets.json` |
...
(ASCII-стейджинг `ProgramData\PhygitalStudio\imports` / `/tmp/phygital-imports`
убран в V1.1 — Pr ест UTF-8 пути напрямую, см. §11.2.)

Содержимое AppData/PhygitalStudio:

| Файл/каталог | Что |
|---|---|
| `session.json` | backend cookies + JWT + captured_at |
| `sidecar.token` | per-install shared-secret панель → sidecar |
| `user_data/` | Playwright persistent profile + `presets.json` |
| `downloads/<job_id>/` | Скачанные результаты |
| `downloads-panel/` | Результаты, сохранённые панелью на диск (disk_save.js) |
| `uploads/<session_id>/` | Загруженные init-картинки |
| `asset_uploads/` | multipart-загрузки панели + ffmpeg-выводы `/extract-frame`, `/clip-video` |
| `asset_cache.jsonl` | sha256 → file_obj_id (backend upload dedupe) |
...
| `FamilyTabs` — Image / Video / Upscale / Voice | Фильтр моделей по семейству |
| `ModelPicker` — список моделей | `GET /nodes` + `GET /nodes/video` + `GET /nodes/upscale` |
| `VersionPicker` (в GenerateTab) | Версия движка (`model_name`/`model`) — дропдаун под Model; параметр исключается из Advanced settings |
| `ScenarioPicker` — список сценариев | Из `scenarios` ноды; hint "Needs: …" по `requiredSlots` |
| `PresetBar` | Сохранение/загрузка пресетов формы (`/presets`) |
...
| `SlotPicker` — «From Timeline frame» | host: source-relative секунда клипа под playhead → `POST /extract-frame` |
| `SlotPicker` — «From Timeline In/Out» | `host.getSourceInOut()` → `POST /clip-video` |
| `SlotPicker` — «Selected clip» (video-слоты) | Выделенный на таймлайне клип → `POST /clip-video` |
| `SlotList` — × на каждом item'е | Удалить из slot'а |
| `PromptInput` | Textarea + char counter / "required"; кнопка Enhance → `POST /enhance` (preview-and-confirm) |
| `ParamsAccordion` | Advanced settings: enum / number / bool; прячет version-параметр и enum'ы с ≤1 опцией (`visibleParamKeys`) |
| `EnumDropdown` | Custom dropdown через `position:fixed` — native `<select>` обрезается CEP-iframe'ом |
...
| `QueueWidget` | Активные job'ы + per-job cancel |
| `JobFilter` chips | Фильтр по статусу с count'ами; muted (40% opacity) при count=0 |
| `JobCard` — «Insert» / «To timeline» | host.importToBin (+ insertClip на playhead) |
| `JobCard` — «Retry» / «Copy prompt» | Восстановить draft из `job.params` / скопировать промпт |
| `JobCard` — «Open folder» | shell open AppData/.../downloads/<job_id> |
| `JobCard` — ⋯ menu — Delete | DELETE /jobs/{id} с подтверждением |
| `JobCard` — inline-плеер | Для Voice TTS (mp3) |
...
- **Sidecar рестарт.** `TaskRegistry` при старте читает `jobs.jsonl` и для каждой
  незавершённой задачи делает poll backend `/api/v2/task/{id}` чтобы
...
- **Asset cache.** `asset_cache.jsonl` (sha256 → file_obj_id) переживает рестарт —
  один и тот же файл не заливается на backend повторно.
...
```bash
# Sidecar
cd sidecar && python -m pytest -q --deselect tests/test_e2e_live.py
                                           # ~274 passed, 2 skipped
python -m pytest -m live -v -s             # требует sidecar + сессии

# Pr panel
cd cep-premiere && npm test                # ~175 passed, 2 skipped (integration)
...
Решение — ffmpeg напрямую из исходника через `/extract-frame` (минуя Pr-рендер).
...
**`/usr/bin/python3` это 3.9.** Avoid. Нужен 3.11+ из Homebrew или python.org.
...
**Content moderation backend возвращает успех + `error_code:"safety"` в
...
**Семафор N=5 на job_runner.** Жёсткий лимит на параллельные задачи в backend
...
- Cost preview для некоторых видео-нод неточен (backend `get_credits_price`
